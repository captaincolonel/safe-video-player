// ==UserScript==
// @name              全网VIP视频免费破解去广告【安全中转稳定版】
// @namespace         video_vip_safe_transit_stable
// @version           3.1.5-safe-stable.1
// @description       基于可正常显示悬浮窗的初始脚本，仅把内嵌播放和弹窗播放改为先进入安全中转页。
...
const superVip = (function () {
    ...
    const _CONFIG_ = {
        ...
        safeTransitPlayerUrl: "https://captaincolonel.github.io/safe-video-player/player.html",
        safeTransitCompatMode: true,
        videoParseList: [
    ...
    function buildSafeTransitUrl(parseUrl) {
        const separator = _CONFIG_.safeTransitPlayerUrl.includes("?") ? "&" : "?";
        const compatParam = _CONFIG_.safeTransitCompatMode ? "compat=1&" : "";
        return _CONFIG_.safeTransitPlayerUrl + separator + compatParam + "url=" + encodeURIComponent(parseUrl);
    }
